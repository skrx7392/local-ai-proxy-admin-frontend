...
/**
 * Focus the box with the global `/` shortcut. The shortcut's document
 * keydown listener is attached in a hydration effect, so a single early
 * press can be dropped; retry until it lands (still exercises the real
 * shortcut path). Returns the input locator.
 */
async function focusViaSlash(page: Page): Promise<Locator> {
...
/** Deterministic open via click (native focus + React onFocus → popover). */
async function openByClick(page: Page): Promise<Locator> {
  const input = page.getByTestId('nav-search');
  await input.click();
...
test.describe('topbar "Go to…" suggestions', () => {
  ...
  test('happy path: `/` opens all destinations, arrow + Enter navigates', async ({
    ...
    const input = await focusViaSlash(page);
  ...
  test('Escape closes the popover and keeps focus; legacy exact-name Enter still works', async ({
    ...
    const reopened = await openByClick(page);
    await page.keyboard.press('Escape');
    await page.keyboard.press('Escape');
    await expect(reopened).not.toBeFocused();
